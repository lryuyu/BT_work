...
public:
  ...
  void onWrite(BLECharacteristic *pCharacteristic) override {
    Serial.println("[BLE] Received write to characteristic");
    const std::string rxValue = pCharacteristic->getValue();
    if (!rxValue.empty()) {
      Serial.printf("[BLE] Received data length: %zu\n", rxValue.length());
      ...
      Serial.printf("[BLE] Comma positions: first=%d, second=%d, third=%d\n", firstComma, secondComma, thirdComma);
      ...
      if (firstComma > 0 && secondComma > firstComma && thirdComma > secondComma) {
        ...
        Serial.printf("[BLE] Config parsed successfully:\n");
        ...
        Serial.printf("[BLE] PORT:%d\n", manager->config.port);

        if (manager->pServer) {
          BLEAdvertising* adv = manager->pServer->getAdvertising();
          if (adv) {
            Serial.println("[BLE] Restarting BLE advertising after config write.");
            adv->start();
          }
        }
      } else {
        Serial.println("[BLE] Invalid config format received via BLE");
        Serial.println("[BLE] Expected format: SSID,PASSWORD,IP,PORT");
        Serial.printf("[BLE] Received format: %s\n", received.c_str());
      }
    } else {
      Serial.println("[BLE] Received empty value via BLE");
    }
  }
};
...
void BluetoothManager::setupBLE() {
  ...
  if (pCharacteristic == nullptr) {
    Serial.println("[BLE] Failed to create BLE characteristic");
    return;
  }
  ...
  if (pAdvertising == nullptr) {
    Serial.println("[BLE] Failed to get BLE advertising");
    return;
  }
  Serial.println("[BLE] BLE advertising obtained");
