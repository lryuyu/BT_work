...
//?????????????
 static void batchSensorTask(void *pvParameters) {
    ...
    while (true) {
        ...
        rawData.pp_0 = simpleFilter(rawData.pp_0 , s_filterStates[5]);
        rawData.pp_1 = simpleFilter(rawData.pp_1 , s_filterStates[6]);
        rawData.head_0 = simpleFilter(rawData.head_0 , s_filterStates[7]);
        rawData.nose_0 = simpleFilter(rawData.nose_0, s_filterStates[8]);
        rawData.nose_1 = simpleFilter(rawData.nose_1, s_filterStates[9]);
...
uint32_t PN532_1_read_uid() {
    ...
    if (PN532.readPassiveTargetID(PN532_MIFARE_ISO14443A, uid, &uidLength)) {
        ...
        if (uidLength != TARGET_UID_LEN)
            return 0;
        for (uint8_t i = 0; i < TARGET_UID_LEN; i++) {
            if (uid[i] != TARGET_UID_0[i])
                return 0;
        }
        return 1;
    }
...
void sensor_read_all_raw(sensor_data_t *data) {
    ...
    data->pp_0   = MCP3008_read(A_SENS_PP_0_PIN.adc,   A_SENS_PP_0_PIN.ch);
    data->pp_1   = MCP3008_read(A_SENS_PP_1_PIN.adc,   A_SENS_PP_1_PIN.ch);
    data->head_0 = MCP3008_read(A_SENS_HEAD_0_PIN.adc, A_SENS_HEAD_0_PIN.ch);
    data->nose_0 = MCP3008_read(A_SENS_NOSE_0_PIN.adc, A_SENS_NOSE_0_PIN.ch);
    data->nose_1 = MCP3008_read(A_SENS_NOSE_1_PIN.adc, A_SENS_NOSE_1_PIN.ch);
